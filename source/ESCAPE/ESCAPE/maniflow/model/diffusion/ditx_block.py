...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, c: torch.Tensor, 
        ...
        if self.fused_attn and (not self.capture_attention):
...
class DiTXBlock(nn.Module):
    ...
    def forward(self, x, time_c, context_c, attn_mask=None):
        ...
        if self.capture_attention and self_attn_weights is not None:
            self.last_self_attn = self_attn_weights.detach()
