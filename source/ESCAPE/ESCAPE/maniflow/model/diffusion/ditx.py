...
class DiTX(nn.Module):
    ...
    def set_attention_capture(self, enabled: bool = True) -> None:
        """Enable/disable storing per-block attention maps during forward."""
        self.capture_attention = enabled
        for block in self.blocks:
            block.capture_attention = enabled
            block.cross_attn.capture_attention = enabled
            if not enabled:
                block.last_self_attn = None
                block.cross_attn.last_attn = None
        if not enabled:
            self._last_attention_maps = {}

    def get_last_attention_maps(
        self,
        reduce_heads: bool = True,
        to_cpu: bool = True,
    ) -> Dict[str, Any]:
        """
        Get the last captured attention maps from all DiTX blocks.
        Returns self-attention and cross-attention maps keyed by block index.
        """
        maps: Dict[str, Any] = {
            "self_attn": {},
            "cross_attn": {},
        ...
        for i, block in enumerate(self.blocks):
            self_attn = block.last_self_attn
            cross_attn = block.cross_attn.last_attn

            if self_attn is not None:
                if reduce_heads:
                    self_attn = self_attn.mean(dim=1)
                if to_cpu:
                    self_attn = self_attn.detach().cpu()
                maps["self_attn"][f"block_{i}"] = self_attn
            ...
            if cross_attn is not None:
                ...
                maps["cross_attn"][f"block_{i}"] = cross_attn
    ...
    def forward(self, 
        ...
        if self.capture_attention:
            # Cache maps after each forward so caller can inspect the most recent step.
            self.get_last_attention_maps(reduce_heads=True, to_cpu=True)
