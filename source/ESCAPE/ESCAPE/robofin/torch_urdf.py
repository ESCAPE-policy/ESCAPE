...
class TorchMesh(Mesh):
    def __init__(
        ...
        self.device = device
        self.vertices = []
        self._meshes = None  # Set _meshes directly to avoid triggering the property setter.
        super().__init__(filename, combine, scale, meshes, lazy_filename)
        # Create vertices after super().__init__ if meshes have been loaded.
    ...
    def meshes(self):
        ...
        if self.lazy_filename is not None and self._meshes is None:
            loaded = self._load_and_combine_meshes(self.lazy_filename, self.combine)
    ...
    def meshes(self, value):
        ...
        #             )
        lazy_filename = getattr(self, 'lazy_filename', None)
        if lazy_filename is not None and value is None:
            self._meshes = None
            return
